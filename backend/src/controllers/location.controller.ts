...
export async function reverseGeocodeController(req: Request, res: Response) {
  ...
  try {
    const result = await reverseGeocodeLocation({ projectId, latitude, longitude }, audience)
...
export async function searchLocationSuggestionsController(req: Request, res: Response) {
  ...
  try {
    const result = await searchLocationSuggestions({ projectId, query: trimmedQuery }, audience)
