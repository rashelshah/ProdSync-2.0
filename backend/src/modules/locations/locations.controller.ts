...
import { canCommentOnLocations, canManageLocations, canUploadLocationMedia, canViewAllLocations, canViewLocations } from './locations.access'
import {
...
function buildLocationScope(req: Request) {
  return {
    viewerUserId: req.authUser?.id ?? null,
    canViewAll: canViewAllLocations(req),
  }
}
...
export async function getLocationDetailController(req: Request, res: Response) {
  ...
  const detail = await getLocationDetail(query.projectId, String(req.params.id ?? ''), buildLocationScope(req))
  res.json(detail)
}
...
export async function listLocationMediaController(req: Request, res: Response) {
  ...
  const media = await listLocationMedia(query.projectId, String(req.params.id ?? ''), query.page, query.pageSize, buildLocationScope(req))
  res.json(media)
}
...
export async function listLocationDocumentsController(req: Request, res: Response) {
  ...
  const documents = await listLocationDocuments(query.projectId, String(req.params.id ?? ''), query.page, query.pageSize, buildLocationScope(req))
  res.json(documents)
}
...
export async function listLocationPermissionsController(req: Request, res: Response) {
  requireViewAccess(req)
  const query = locationsProjectQuerySchema.parse(req.query)
  const permissions = await listLocationPermissions(query.projectId, String(req.params.id ?? ''), buildLocationScope(req))
...
export async function listLocationTimelineController(req: Request, res: Response) {
  requireViewAccess(req)
  const query = locationsProjectQuerySchema.parse(req.query)
  const timeline = await listLocationTimeline(query.projectId, String(req.params.id ?? ''), buildLocationScope(req))
  res.json({ timeline })
}
...
export async function listLocationCommentsController(req: Request, res: Response) {
  requireViewAccess(req)
  const query = locationsProjectQuerySchema.parse(req.query)
  const comments = await listLocationComments(query.projectId, String(req.params.id ?? ''), buildLocationScope(req))
  res.json({ comments })
}
...
export async function listLocationCostsController(req: Request, res: Response) {
  requireViewAccess(req)
  const query = locationsProjectQuerySchema.parse(req.query)
  const costs = await listLocationCosts(query.projectId, String(req.params.id ?? ''), buildLocationScope(req))
  res.json({ costs })
}
