/**
 * CircularGallery — react-bits WebGL horizontal gallery
 * ──────────────────────────────────────────────────────────────────────────────
 * Source: react-bits CircularGallery (TypeScript + Tailwind)
 * Modifications vs original:
 *   1. onResize() — plane.scale.x WIDER, plane.scale.y SHORTER → 2.5 : 1 ratio
 *   2. Default font changed to "600 18px Inter" for premium module labels
 *   3. Title mesh Y-offset increased slightly for breathing room
 * Core WebGL logic (Renderer, Camera, App, Media, Title) is UNCHANGED.
 */
...
function createTextTexture(
  ...
  // Scale factor for high DPI/crisp text rendering
  const scaleMultiplier = 3;
  const baseFontSize = getFontSize(font);
  const scaledFontSize = baseFontSize * scaleMultiplier;
  const scaledFont = font.replace(`${baseFontSize}px`, `${scaledFontSize}px`);

  context.font = scaledFont;
  const metrics = context.measureText(text);
  const textWidth = Math.ceil(metrics.width);
  const textHeight = Math.ceil(scaledFontSize * 1.2);

  canvas.width = textWidth + (10 * scaleMultiplier);
  canvas.height = textHeight + (10 * scaleMultiplier);
  ...
  // Optional text shadow for even better legibility against varied backgrounds
  context.shadowColor = 'rgba(0, 0, 0, 0.4)';
  context.shadowBlur = 4 * scaleMultiplier;
  context.shadowOffsetY = 2 * scaleMultiplier;
...
class Title {
  ...
  constructor({ gl, plane, renderer, text, textColor = '#ffffff', font = '600 22px Inter' }: TitleProps) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
  ...
  createMesh() {
    ...
    // Scale text proportionally - slightly smaller factor because cards are much larger now
    const textHeightScaled = this.plane.scale.y * 0.08; 
    const textWidthScaled = textHeightScaled * aspect;
    this.mesh.scale.set(textWidthScaled, textHeightScaled, 1);
    
    // Push text further down, below the card shadow
    this.mesh.position.y = -this.plane.scale.y * 0.5 - textHeightScaled * 0.5 - 0.25;
...
class Media {
  ...
  onResize({ screen, viewport }: { screen?: ScreenSize; viewport?: Viewport } = {}) {
    ...
    // ── HORIZONTAL CARD SIZING (16:9 ratio, same shape as hero preview) ──────────
    // Increased size relative to viewport for bigger cards.
